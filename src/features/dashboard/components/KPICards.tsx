  ...
  return (
    <div className="col-span-12 grid grid-cols-1 md:grid-cols-4 gap-card-gap">
      {kpis.map((kpi, idx) => (
        <div
          key={idx}
          className="bg-surface-container-lowest rounded-lg border border-outline-variant shadow-sm p-4 flex flex-col justify-between"
        >
          ...
          <div className="flex items-end gap-2">
            <span className="text-display text-on-surface">{kpi.value}</span>
            <span className="text-body-md text-on-surface-variant pb-1">
              {kpi.unit}
            </span>
          </div>
          <div
            className={`mt-2 flex items-center gap-1 ${kpi.trendColor} text-label-sm`}
          >
            <kpi.TrendIcon
              size={14}
              aria-label={
                kpi.trendColor.includes("success")
                  ? "Positive trend"
